...
private:
  ...
  static void need_data(GstElement * appsrc, guint /*unused*/, gpointer user_data)
  {
    ...
    GST_BUFFER_DURATION(buffer) =
      gst_util_uint64_scale_int(1, GST_SECOND, self->fps_);
  ...
  static void media_configure(
    GstRTSPMediaFactory * /*factory*/, GstRTSPMedia * media,
    gpointer user_data)
  {
    auto * self = static_cast<RtspStreamNode *>(user_data);
    ...
    GstCaps * caps = gst_caps_new_simple(
      ...
      "width", G_TYPE_INT, self->width_,
      "height", G_TYPE_INT, self->height_,
      "framerate", GST_TYPE_FRACTION, self->fps_, 1,
      nullptr);
    g_object_set(
      appsrc,
      "caps", caps,
  ...
  void start_rtsp_server()
  {
    gst_thread_ = std::thread([this]() {
      ...
      RCLCPP_INFO(
        get_logger(), "RTSP server at rtsp://0.0.0.0:%d%s",
        port_, mount_point_.c_str());
